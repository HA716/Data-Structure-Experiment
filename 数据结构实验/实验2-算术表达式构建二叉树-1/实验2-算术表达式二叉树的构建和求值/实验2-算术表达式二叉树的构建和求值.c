...
//本可以用数组表示指针数组ASItem,ASSign
BTNode* creat_tree(char *str)
{
	int itemCount = 0, signCount = 0, len = strlen(str), i; //AS意为addSub加减法,前者为加减项计数,后者为加减符号计数,用于数组下标
	...
	for (i = 1; i < signCount; i++)             //以加减法符号节点作为子树根节点，加减法之间的项的节点为子树根节点的孩子节点
