...
void ShowSon(TreeNode *node, string name) {
	...
	TreeNode *temp = NULL;
	if (parent->lChild == NULL) {//bug这样有要是你的兄弟没老婆，会把兄弟当成孩子的 
...
int main() {
	...
	while (true) {
		...
		if (choose1 == '1') {
			while (true) {
				char choose2 = '-1';
				cout << "1--输入家谱，2--输出家谱，3--清除记录返回，4--保存记录后返回：";
				cin >> choose2;
				if (choose2 == '1') {
					if (flag == false) {
						save(ancestor);
			...
			while (true) {
				char choose3 = '-1';
				cout << "1--括号表示法输出，2--查找某人的所有儿子，3--查找某人的所有祖先，4--返回：";
				cin >> choose3;
				if (choose3 == '1') {
					if (ancestor == NULL) {
						cout << "家谱为空" << endl;
